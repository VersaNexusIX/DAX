# ─────────────────────────────────────────────────────────────────
#  DAX  —  Makefile
#  Auto-detects: OS, arch, compiler (GCC vs Clang), install prefix
# ─────────────────────────────────────────────────────────────────
...
# ── Per-platform defines, ASM, and install prefix ────────────────
ifeq ($(IS_ANDROID), yes)
...
# ── Targets ──────────────────────────────────────────────────────
.PHONY: all clean install info _banner

all: _banner $(TARGET)
	@printf '$(GREEN)$(BOLD)\n'
	@printf '  ╔═══════════════════════════════════════════╗\n'
	@printf '  ║  ✓  DAX v3.0.0  built successfully       ║\n'
	@printf '  ║                                           ║\n'
	@printf '  ║  $(CYAN)./$(TARGET)$(GREEN) $(GREY)<binary>$(GREEN)               run      ║\n'
	@printf '  ║  $(CYAN)./$(TARGET)$(GREEN) $(GREY)-h$(GREEN)                     help     ║\n'
	@printf '  ║  $(CYAN)./$(TARGET)$(GREEN) $(GREY)-x -i <binary>$(GREEN)         RE mode  ║\n'
	@printf '  ╚═══════════════════════════════════════════╝\n'
	@printf '$(RESET)\n'
...
clean:
	...
	@rm -f src/*.o arch/*.o $(TARGET) dax.exe
