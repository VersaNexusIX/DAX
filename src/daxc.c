...
int dax_daxc_write(dax_binary_t *bin, dax_opts_t *opts, const char *path) {
    ...
    fp = fopen(path, "wb");
    if (!fp) { perror("dax: cannot create .daxc"); return -1; }
    ...
    if (opts && opts->verbose)
        fprintf(stdout, "  [DAXC] Written: %s  (insns=%llu syms=%d funcs=%d)\n",
                path, (unsigned long long)ninsns, bin->nsymbols, bin->nfunctions);
    return 0;
}
...
int dax_daxc_read(const char *path, dax_binary_t *bin) {
    ...
    fp = fopen(path, "rb");
    if (!fp) { perror("dax: cannot open .daxc"); return -1; }

    if (fread(&hdr, sizeof(hdr), 1, fp) != 1) { fclose(fp); return -1; }
    if (hdr.magic != DAX_DAXC_MAGIC) {
        fprintf(stderr, "dax: not a .daxc file\n");
        fclose(fp);
        return -1;
    }
...
int dax_daxc_to_asm(const char *daxc_path, const char *asm_path, int color) {
    ...
    if (asm_path) {
        ...
        if (!out) { perror("dax: cannot create .S"); fclose(fp); return -1; }
        c = 0;
    } else {
        out = stdout;
    }
