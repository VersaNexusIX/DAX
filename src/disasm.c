...
static const char *dax_resolve_string(dax_binary_t *bin, uint64_t addr) {
    ...
    for (i = 0; i < bin->nsections; i++) {
        ...
        /* Must start with a printable or common escape char */
        if (!isprint((unsigned char)p[0]) &&
            p[0] != '\0' && p[0] != '\n' && p[0] != '\t') continue;
        if (p[0] == '\0') continue;  /* empty string — skip */

        len = 0;
        while (len < left && len < 256) {
        ...
        /* Require NUL terminator, minimum length 2 */
        if (len < 2)                       continue;
        if (len >= left)                   continue;  /* no NUL found */
        if (p[len] != '\0')               continue;
...
int dax_disasm_x86_64(dax_binary_t *bin, dax_opts_t *opts, FILE *out) {
    ...
    if (!sec || sec->offset + sec->size > bin->size) {
        fprintf(stderr, "dax: section not found or out of bounds\n");
        return -1;
    }
